...
const cx = (...parts: (string | undefined | null | false)[]): string => parts.filter(Boolean).join(' ');

const useResizeObserver = (callback: () => void, elements: RefObject<HTMLElement>[], dependencies: any[]) => {
  useEffect(() => {
...
const useImageLoader = (seqRef: RefObject<HTMLElement>, onLoad: () => void, dependencies: any[]) => {
  useEffect(() => {
...
const useAnimationLoop = (trackRef: RefObject<HTMLElement>, targetVelocity: number, seqWidth: number, isHovered: boolean, pauseOnHover: boolean) => {
